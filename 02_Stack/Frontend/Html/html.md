...
### input 
Để lấy dữ liệu khi người dùng nhập vào. Thẻ input không có thẻ đóng
**Syn**
```bash
...
```
Thuộc tính value
Để xác định giá trị mặc định cho ô đầu vào.
Cú pháp: value= “…”;
Thuộc tính name
Để xác tên cho ô đầu vào, từ đó xác định trường dữ liệu khi người dùng submit thông tin.
Cú pháp: name = “…”;
Thuộc tính placeholder
Để đặt một văn bản mẫu cho ô đầu vào, từ đó người dùng biết được nội dung cần nhập vào ô.
Cú pháp: placeholder = “…”;
Thuộc tính required
Hệ thống sẽ ghi nhận đây là ô dữ liệu bắt buộc phải nhập trước khi submit file HTML lên sever.
Cú pháp: <input required>
Thuộc tính disabled
Nếu giá trị là true thì đồng nghĩa người dùng không thể nhập hay thao tác với ô này.
Cú pháp: <input disabled>
Thuộc tính size
Xác định độ rộng của ô đầu vào, giá trị là number + đơn vị
Thuộc tính max-length
Xác định số kí tự tối đa mà người dùng được phép nhập vào ô
Thuộc tinhd min & max
Xác định giá trị tối thiểu hay tối đa khi nhập ô đầu vào
Thuộc tính autocomplete
Gợi ý các lựa chọn để người dùng nhập dữ liệu nhanh và dễ dàng hơn
Thuộc tính step
Thường được sử dụng trong cá loại nhập số, dùng để xác định giá trị bước nhảy của ô
Ví dụ tạo trường đăng nhập (Username)
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
