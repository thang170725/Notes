- [Javascript](#javascript)
  ...
  - [API trong Javascript](#api-trong-javascript)
    - [Fetch GET](#fetch-get)
      - [Call API để  GET DATA JSON lên giao diện](#call-api-để--get-data-json-lên-giao-diện)
  - [5](#5)
  - [3](#3)
- [Gán](#gán)
...
### Fetch GET
Là một API dùng để gửi các yêu cầu HTTP (GET, POST, PUT, DELETE, …) đến server và xử lý kết quả trả về. Nó thay thế cho XMLHttpRequest trước đây, và là một phần của Fetch API, được tích hợp sẵn trong trình duyệt hiện tại.
**Cú pháp**
```javascript
...
```
#### Call API để  GET DATA JSON lên giao diện
**HTML**
```html
<button onclick="getData('left')">Display content on the left</button>
<button onclick="getData('right')">Display content on the right</button>
```
**Javascript**
```js
async function getData(side) { 
    try {
        const response = await fetch(`http://127.0.0.1:8000/user/data-${side}`);
...
async/await
    • Dùng để xử lý các tác vụ bất đồng bộ (asynchoronous) một cách dễ đọc hơn so với dùng .then() và catch() của Promise. Nó cho phép dùng await bên trong → tức là có thể chờ kết quả trả về từ server mà không làm ‘đơ’ trình duyệt.
    • Đây là cách xử lý API hiện đại, ra từ ES2017
Cú pháp:
...
Từ khóa async biến hàm thành hàm bất đồng bộ, cho phép dùng await bên trong.
await giúp chờ Promise hoàn thành (tương đương .then()) nhưng viết giống code tuần tự.
try...catch bắt lỗi, tương đương .catch().
...
sayHello();
innerText
Thiết lập hoặc trả về nội dung văn bản của một phần tử.
Sự khác nhau giữa innerHTML và innerText
Chúng đều được sử dụng để thao tác với nội dung.
innerText
innerHTML
Chỉ tao tác với văn bản thuần túy mà người dùng nhìn thấy trên trang web. Nó bỏ qua các thẻ HTML, các phần tử ẩn
Cho phép truy cập và thay đổi mã HTML bên trong phần tử. Khi đặt giá trị cho innerHTML, trình duyệt sẽ phân tích cú pháp của nó như mã HTML và hiển thị kết quả
function main(){
    let t = document.getElementsByTagName("div")[0];
    t.innerText = ` <p style="display: none;">Nội dung ẩn</p>
                    <p>Nội dung hiển thị,
                        <strong>nội dung nhấn mạnh</strong>
                    </p>`;
}
main();
function main(){
    let t = document.getElementsByTagName("div")[0];
    t.innerHTML = ` <p style="display: none;">Nội dung ẩn</p>
                    <p>Nội dung hiển thị,
                        <strong>nội dung nhấn mạnh</strong>
                    </p>`;
}
